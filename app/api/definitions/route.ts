...
export async function GET() {
  ...
  try {
    const res = await fetch(`${e.url}/rest/v1/definitions?select=*&order=updated_at.desc,nullsLast&limit=50`, {
    ...
    console.error("/api/definitions GET error", err)
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const res = await fetch(`${e.url}/rest/v1/definitions`, {
      method: "POST",
      headers: {
        apikey: e.key,
