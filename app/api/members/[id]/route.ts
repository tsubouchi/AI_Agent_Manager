...
const updateSchema = z.object({ name: z.string().optional(), avatar_url: z.string().url().optional() })
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const res = await fetch(`${e.url}/rest/v1/members?id=eq.${params.id}`, {
      method: "PATCH",
      headers: {
        apikey: e.key,
    ...
    console.error("/api/members/[id] PATCH error", err)
